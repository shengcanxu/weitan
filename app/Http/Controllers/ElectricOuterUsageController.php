...
class ElectricOuterUsageController extends Controller
{
    ...
    public function store(ElectricOuterUsageRequest $request){
        ...
        \WeitanHelper::log("新建了id=".$usage->id."的电力使用外部记录",$request->user());
    ...
    public function change(ElectricOuterUsageRequest $request, $id){
        ...
        if($usage != null) {
            ...
            \WeitanHelper::log("修改了id=".$id."的电力使用外部记录",$request->user());
    ...
    public function delete(Request $request, $id){
        ...
        if($store != null){
            $store->delete();
            \WeitanHelper::log("删除了id=".$id."的电力使用外部记录",$request->user());
            return response()->json(['status'=>'success']);
        }
    ...
    public function tagerror(TagErrorRequest $request, $id){
        ...
        if($store != null){
            ...
            \WeitanHelper::log("对id=".$id."的电力使用外部记录标记错误信息：".$store->errorinfo,$request->user());
            \WeitanHelper::reportError("电力使用外部记录", $store->errorinfo, $store->author, $request->user()->id );
            return response()->json(['status'=>'success']);
        }
