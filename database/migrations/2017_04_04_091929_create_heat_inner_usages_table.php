...
class CreateHeatInnerUsagesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create("heat_inner_usages",function(Blueprint $table){
            $table->increments('id');
            $table->date('month'); //月份
            $table->string('producetype'); //生产工序
            $table->string('device'); //计量仪器
    ...
    {
        Schema::dropIfExists('heat_inner_usages');
    }
}
